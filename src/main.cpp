...
// Copyright (c) 2011-2015 The Sparklecoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_SPKL;
...
const string strMessageMagic = "Sparklecoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // sparklecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // sparklecoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // sparklecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// sparklecoin: find block wanted by given orphan block
...
// sparklecoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // sparklecoin: target change every block
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // sparklecoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // sparklecoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // sparklecoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // sparklecoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // sparklecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // sparklecoin: track money supply and mint amount info
    ...
    // sparklecoin: fees are not collected by miners as in bitcoin
    // sparklecoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// sparklecoin: total coin age spent in transaction, in the unit of coin-days.
...
// sparklecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // sparklecoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // sparklecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // sparklecoin: record proof-of-stake hash value
    ...
    // sparklecoin: compute stake modifier
...
bool CBlock::CheckBlock(int64 nBlockHeight) const
{
    ...
    // sparklecoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // sparklecoin: coinbase output should be empty if proof-of-stake block
    ...
    if (vtx[0].GetValueOut() > (IsProofOfWork()? (GetProofOfWorkReward(nBits, nBlockHeight, nTime) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0))
    ...
    // sparklecoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // sparklecoin: check that the block satisfies synchronized checkpoint
    ...
    // sparklecoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // sparklecoin: check proof-of-stake
    ...
    // sparklecoin: verify hash target and signature of coinstake tx
    ...
    // sparklecoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // sparklecoin: getblocks may not obtain the ancestor block rejected
    ...
    // sparklecoin: if responsible for sync-checkpoint send it
...
// sparklecoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Sparkleunity", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Sparkleunity", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "22-Feb-2017 NASA announced thrilling discovery of 7 Earth sized Planets";
        CTransaction txNew;
        txNew.nTime = 1501771249;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501772249;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 376721577;

        if (fTestNet)
        {
            block.nTime    = 1501771349;
            block.nNonce   = 87305;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x203383930ac9261e213c00e0b9afb7b2d94d56515fd3c7b9f255c8094269bb27"));
        block.print();
        ...
        // sparklecoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // sparklecoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // sparklecoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // sparklecoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // sparklecoin: should not enter safe mode for longer invalid chain
    // sparklecoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // sparklecoin: if detected invalid checkpoint enter safe mode
    ...
    // sparklecoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // sparklecoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // sparklecoin: record my external IP reported by peer
        ...
        // sparklecoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // sparklecoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // sparklecoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // sparklecoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // sparklecoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // sparklecoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
