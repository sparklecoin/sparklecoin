...
Object blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    uint256 hashBlock = blockindex->GetBlockHash();
    ...
    if (hashBlock != 0)
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second)
        {
            ...
            if (pindex->IsInMainChain())
            {
                result.push_back(Pair("confirmations", 1 + nBestHeight - pindex->nHeight));
    ...
    result.push_back(Pair("chaintrust", blockindex->bnChainTrust.GetHex()));
...
// sparklecoin: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Sparklecoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Sparklecoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <sparklecoinaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <sparklecoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <sparklecoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <sparklecoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <sparklecoinaddress> [minconf=1]\n"
            "Returns the total amount received by <sparklecoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <tosparklecoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Sparklecoin address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // sparklecoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <sparklecoinaddress>\n"
            "Return information about <sparklecoinaddress>.");
...
Value getblockheader(const Array& params, bool fHelp)
{
    ...
    bool fVerbose = params.size() > 1 ? params[1].get_bool() : true;
...
Value getblock(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            "getblock <hash> [verbose] [txinfo] [txdetails]\n"
    ...
    bool fVerbose = params.size() > 1 ? params[1].get_bool() : true;
    bool fTxInfo = params.size() > 2 ? params[2].get_bool() : false;
    bool fTxDetails = params.size() > 3 ? params[3].get_bool() : false;
...
// sparklecoin: get information of sync-checkpoint
...
// sparklecoin: reserve balance from being staked for network protection
...
// sparklecoin: check wallet integrity
...
// sparklecoin: make a public-private key pair
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a sparklecoin address or hex-encoded public key\n"
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\" (string, required) A json array of keys which are sparklecoin addresses or hex-encoded public keys\n"
            " [\n"
            " \"key\" (string) sparklecoin address or hex-encoded public key\n"
            " ,...\n"
            " ]\n"
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "getblockheader",         &getblockheader,         true },
};
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: sparklecoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "getblock"               && n > 3) ConvertTo<bool>(params[3]);
